...
export default function HomePage(){
  ...
  useEffect(() => {
    ...
    const fetchTopSaleList = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/product/get-top-sale-list');
  ...
  useEffect(() => {
    ...
    const fetchNewList = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/product/get-new-list');
  ...
  return(
    <div className = "HomePage">
      <BriefProductShowcase productLists = {topSaleList} title= "Các sản phẩm bán chạy nhất"/>
