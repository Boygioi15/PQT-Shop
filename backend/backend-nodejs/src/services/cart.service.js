...
export class CartService {
    static async addToCartFromLocal({ userId, carts = [] }) {
        if (!carts.length) {
    ...
    static async addToCart({ userId, skuId, quantity }) {
    ...
    static async addToCartV2({ userId, item_products }) {
        const { quantity, old_quantity, skuId } = item_products;
    ...
    static async deleteUserCart({ userId, skuId }) {
        const query = {
                cart_userId: userId,
                cart_state: 'active',
            },
    ...
    static async replaceItemInCart({ userId, oldSkuId, newSkuId }) {
        ...
        let newProduct = await findSkuById(newSkuId);
    ...
    static getProductInforForLocal = async ({ carts }) => {
        const items = await Promise.all(
